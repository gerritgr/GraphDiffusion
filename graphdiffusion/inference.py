...
class VectorInference(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        if (
            len(steps) < 2
            or not np.all(steps[:-1] > steps[1:])
            or not (0 <= steps[0] <= 1 and 0 <= steps[-1] <= 1)
        ):
            raise ValueError(
                "Steps must be a decreasing sequence in the range [0, 1] and have a length of at least 2.",
                "steps:",
                steps,
            )
        ...
        for i, t in enumerate(steps):
            ...
            if i >= len(steps) - 1:
                if t < 1e-3:
                    ...
                    return (
                        data_t,
                        data_0,
                    )  # pipeline.bridge(data_t, data_0, t, t) # or just return data_t
            t_next = steps[i + 1]
            assert t_next < t
            ...
            if torch.isnan(data_t).any() or torch.isinf(data_t).any():
                warn_str = (
                    "The tensor contains NaN or Inf values. These will be replaced with 0. step:"
                    + str(i)
                )
                print(warn_str)
                data_t = torch.where(
                    torch.isnan(data_t) | torch.isinf(data_t),
